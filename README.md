...
app/
  bot/                 # LINE 事件處理
  core/                # 設定與常數
  fanfan_core/         # 舊版翻翻君核心模組化層（語言、群組、格式、選單）
  db/                  # 資料庫連線與模型
  repositories/        # 資料存取層
  services/            # 商業邏輯層
  ui/                  # 選單與訊息格式
...
- 已將舊版翻翻君行為抽為 `app/fanfan_core/` 模組
- `handlers` 現在以模組化方式調用舊版核心（語言解析、群組切換、輸出格式、語言設定卡）
- 之後 VIP 版本只需覆寫設定與 UI，不需再改翻譯核心流程
