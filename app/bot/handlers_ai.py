...
from linebot.v3.messaging import TextMessage, FlexMessage, ApiClient, Configuration, MessagingApi, ReplyMessageRequest
        ...
        with ApiClient(configuration) as api_client:
            ...
            messaging_api.reply_message(
                ReplyMessageRequest(
                    ...
                    messages=[TextMessage(text="❌ AI 服務暫時不可用")],
    ...
    try:
        ...
        with ApiClient(configuration) as api_client:
            ...
            messaging_api.reply_message(
                ReplyMessageRequest(
                    replyToken=reply_token,
                    messages=[TextMessage(text=f"❌ 發生錯誤：{str(e)[:50]}")],
                    notificationDisabled=False,
                )
            )
    finally:
    ...
    try:
        ...
        with ApiClient(configuration) as api_client:
            ...
            messaging_api.reply_message(
                ReplyMessageRequest(
                    replyToken=reply_token,
                    messages=[TextMessage(text=f"❌ 錯誤：{str(e)[:50]}")],
                    notificationDisabled=False,
                )
            )
